...
## Prompt 85 — 2026-09-10
**Input:** remove the region from the CPASU visual from HES Forecasting page.make it like how i attached the pic- fy25,fy26,fy27 [screenshot: dark ComposedChart, X-axis FY25/FY26/FY27, ASU/SR grouped bars on the left axis, a violet CPASU line on a 0-0.6 right axis]

**Action:**
- "CPASU Trend" (Visual1 in `AsuSrTrendLayer.jsx`, HES Forecasting Layer 03) dropped its region-grouped default view and click-to-drill-into-a-region mechanic entirely — the X-axis is now plain fiscal period (FY25/FY26/FY27 by default, or Quarter/Month/Week per the page's own View By toggle), with ASU/SR bars and a CPASU line, matching the attached screenshot exactly.
- Reused the existing `cpasuByFY(filters, granularity)` selector directly instead of writing a new one — it already returned the exact `{period, asu, sr, cpasu}` shape needed and already respected the page's granularity toggle.
- Removed the now-fully-dead `cpasuByRegion`, `cpasuTrendByRegion`, `regionTrendGranularity`, and their private helpers (`REGION_SHARE`, `periodsPerYear`) from `tsaData.js`, plus `IMPACT_REGIONS` (its last remaining consumer) and the now-unused `periodsForGranularity` import — all confirmed via grep to have zero other consumers. Also removed `HOLIDAY_REGION_MAP` and the now-unused `PillButton` import from `AsuSrTrendLayer.jsx`.
